...
public class TelegramConfigActivity extends ActivityConfigImpl<TelegramConfig> {
    ...
    public ConfigTestResult testConfig(TelegramConfig config) {
        ...
        try {
            ...
            TelegramBot bot = new TelegramBot(config.getUsername(), config.getToken());
            ...
            return ConfigTestResult.builder()
                    .error(false)
                    .success(true)
                    .message("Telegram Bot Registered and running to use you bot search for t.me/" + config.getUsername())
